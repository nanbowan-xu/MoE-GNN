...
def get_smile_list_from_sdf(mols, name='smile', target_name=['target']):
    ...
    for idx, mol in tqdm(enumerate(mols)):
        ...
        # print(mol)
        smiles_list += [mol.GetProp('smile')]
        # print([float(mol.GetProp('target'))])
...
if __name__ == "__main__":
    ...
    mols = Chem.SDMolSupplier('%s/%s.sdf' % (args.data_dir, args.dataset))
    ...
    out_path = '%s/%s_2d_processed.pkl' % (args.data_dir,  args.dataset)
    ...
    out_path = '%s/%s_3d_processed.pkl' % (args.data_dir, args.dataset)
    with open(out_path, 'wb') as f:
        pickle.dump((data_mols, logp_list), f)
