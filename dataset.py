...
class ComplexDataset(BaseDataset):
    ...
    def build_graph(self, mol):
        ...
        a2a_graph = pgl.Graph(a2a_edges, num_nodes=num_atoms, node_feat={"feat": features,"coord":coords}, edge_feat={"dist": dist_feat})
        ...
        dist_grapha = dist_mat.copy()
        dist_grapha[dist_grapha >= 500] = 0.
        dist_grapha = coo_matrix(dist_grapha)
        b2a_edges = list(zip(dist_grapha.row, dist_grapha.col))
        b2a_graph = pgl.Graph(b2a_edges, num_nodes=num_atoms, node_feat={"feat": features})
        ...
        for ind in range(self.num_angle):
            b2b_graph = pgl.Graph(b2b_edges_list[ind], num_nodes=num_bonds, node_feat={"feat":b_coord},edge_feat={"angle": b2b_angle_list[ind]})
...
def global_to_local(R, t, q):
    ...
    q = q.transpose([1, 0])   # (N, L, *, 3) -> (N, L, 3, *)
    if t is None: 
        p = paddle.matmul(R.transpose([1, 0]), q)  # (N, L, 3, *)
    else: 
        p = paddle.matmul(R.transpose([1, 0]), (q - t.unsqueeze(-1))) # (N, L, 3, *)
    p = p.transpose([1, 0])   # (N, L, 3, *) -> (N, L, *, 3) -> (N, L, ..., 3)
    return p
...
def collate_fn(batch):
    ...
    b2a_g = pgl.Graph.batch(b2a_gs).tensor()
    aa=[]
    for i in a2a_gs:
        xx=paddle.to_tensor(i.node_feat['coord'],dtype='float32')
        R = construct_3d_basis(xx[0], xx[1], xx[2])  # (N, L, 3, 3)
        t = xx[0]  # (N, L, 3)
        crd14 = global_to_local(R, t, xx)    # (N, L, 14, 3)
        aa.append(crd14)
    xxx = paddle.concat(aa,axis=0)
    ...
    return a2a_g, b2a_g, b2b_gl,xxx, feats, types, counts, labels
