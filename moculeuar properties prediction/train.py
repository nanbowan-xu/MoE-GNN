...
def evaluate(model, loader, task):
    ...
    for _ in range(loader.steps):
        __, graph_3d, y = loader.next_batch()
        y_hat = model( graph_3d)
...
def train(args, model, trn_loader, tst_loader, val_loader, output_dir_fold):
    ...
    for epoch in range(1, int(args.all_steps/epoch_step) + 1):
        ...
        for _ in range(trn_loader.steps):
            __, graph_3d, y = trn_loader.next_batch()
            y_hat = model(graph_3d)
            ...
            scheduler.step()
            sum_loss += loss
            # print ('check weight: ', model.mpnn_3d.edge2edge_layers[0].conv_layer[0].G.weight)
...
if __name__ == "__main__":
    ...
    for idx in range(args.num_folds):
        ...
        model = EMAILmodel(node_in_dim, edge_in_dim, atom_in_dim, args.rbf_dim, args.hidden_dim, \
