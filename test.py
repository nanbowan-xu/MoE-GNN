...
def evaluate(model, loader):
    ...
    for batch_data in loader:
        ...
        _, y_hat,loss1 = model(a2a_g, b2a_g, b2b_gl, types, counts,y)
        y_hat_list += y_hat.tolist()
        y_list += y.tolist()

    y_hat = np.array(y_hat_list).reshape(-1,)
    y = np.array(y_list).reshape(-1,)
    return y,y_hat,rmse(y, y_hat), mae(y, y_hat), sd(y, y_hat), pearson(y, y_hat)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', type=str, default='./data/')
    parser.add_argument('--dataset', type=str, default='pdbbind2016')
    parser.add_argument('--model_dir', type=str, default='./home-14145')
    ...
    parser.add_argument('--num_angle', type=int, default=1)
    ...
    y,y_hat,rmse_tst, mae_tst, sd_tst, r_tst= evaluate(model, tst_loader)
    file = open('y12.csv', 'w')
    writer = csv.writer(file)

    writer.writerows([y])
    file.close()
    file = open('y_hat12.csv', 'w')
    writer = csv.writer(file)
