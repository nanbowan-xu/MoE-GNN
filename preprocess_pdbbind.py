...
def gen_feature(path, name, featurizer):
    ...
    # print("gg",pocket)
    # pocket = next(pybel.readfile('mol2' ,'%s/%s/%s_pocket.pdb' % (path, name, name)))
...
def pairwise_atomic_types(path, processed_dict, atom_types, atom_types_):
    ...
    for k in processed_dict.keys():
        print ('processed_dict key: ', k)
    print ('all name: ', os.listdir(path))
    for name in tqdm(os.listdir(path)):
        ...
        structure = parser.get_structure('name', '%s/%s/%s_protein.pdb' % (path, name, name))
        ppb = PDB.PPBuilder()
        # print("hh",len(structure))
        for pp in ppb.build_peptides(structure):
            squence = pp.get_sequence()
        ...
        for x, y in zip(ligs, pocks):
            ...
            fea_dict[(y, x)] += 1
        if name in ['1i1e','2f2h','3f8e']:
            continue    
        processed_dict[name]['type_pair'] = list(fea_dict.values())
        processed_dict[name]['squ'] = str(squence)



        m = Chem.MolFromMol2File( '%s/%s/%s_ligand.mol2' % (path, name, name))
        # print("fff",m)
        if m ==None:

            processed_dict[name]['smile'] = 'None'
        else:
        ...
        # processed_dict[name]['squ'] = str(squence)
            processed_dict[name]['smile'] = str(smile)
        # print("kkk",(processed_dict[name]['squ']))
...
def cons_lig_pock_graph_with_spatial_context(ligand, pocket, add_fea=2, theta=5, keep_pock=False, pocket_spatial=True):
    ...
    pock_fea, pock_coord, pock_atoms_raw, pock_edge,squ,smile = pocket
    ...
    if  len(lig_atoms) ==  lig_size and len(pock_atoms) == pock_size:
        ...
        # print("ll",feas)
        return lig_size, coords, feas, atoms_raw,squ,smile
    else:
        return 1.
...
def process_dataset(core_path, refined_path, dataset_name, output_path, cutoff):
    if os.path.exists('/home/wanxu/signnb/data/data.npy'):
        processed_dict = np.load('/home/wanxu/signnb/data/data.npy').item()
    else:
        ...
        np.save('/home/wanxu/signnb/data/data.npy', processed_dict)
    ...
    for k, v in tqdm(data_dict.items()):
        ...
        pocket = (v['pock_fea'], v['pock_co'], v['pock_atoms'], v['pock_eg'],v['squ'],v['smile'])
        print(456789456789)
